...
// draft. scene 0 shows rockets being absorbed animation from RocketsLose.jsx
export default function UfosWin2() {
  ...
  useFrame((state, delta) => {
    ...
    earth.current.position.z = Math.cos(time) * 0.15
    earth.current.rotation.x = Math.cos(time) * 0.05
    earth.current.rotation.z = Math.cos(time) * 0.05
  ...
  return <group>
    ...
    <group name='scene-1'>
      <group ref={earth}>
        <Earth showParticles={false} animate={false} scale={1.7} rotation={[-Math.PI/2, 0, 0]}/>
      </group>
      <group ref={ufo0}>
        <UfoNew position={[0, 7, -1]} scale={3}/>
        {rings.map((value, index) => {
          return <group ref={value} rotation={[Math.PI/3, 0, 0]} position={[0, 0, 0]}>
            <mesh>
