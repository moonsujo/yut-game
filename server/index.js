      ...
      try {
        ...
        if (userFound.roomId === data.fullDocument.shortId && userFound.connectedToRoom) {
          ...
          if (serverEvent.name === "gameStart") {
            ...
            //   gameLogs: data.fullDocument.gameLogs, // only the logs that were generated by the event
            ...
            io.to(userSocketId).emit("passTurn", {
              ...
              newPlayer: room.turn.players[room.turn.team],
              throwCount: room.teams[room.turn.team].throws,
              timeExpired: serverEvent.content.timeExpired,
              turnExpireTime: serverEvent.content.turnExpireTime,
              gamePhase: room.gamePhase,
          ...
          } else if (serverEvent.name === "throwYoot") {
          ...
          } else if (serverEvent.name === "timeExpired") {
            io.to(userSocketId).emit("timeExpired", { 
              teamIndex: room.turn.team,
              playerIndex: room.turn.players[room.turn.team]
            })
          } else {
            io.to(userSocketId).emit('room', roomPopulated)
          }
...
io.on("connect", async (socket) => {
  ...
  // turnExpireTime: with Date.now()
  function startTimer(room, turnExpireTime) {
  ...
  async function switchTurnByTimeExpired(roomId) {
    ...
    if (room.gamePhase === 'pregame') {
      ...
      console.log('[switchTurnByTimeExpired] outcomePregame', outcomePregame)
      if (outcomePregame === "pass") {
        ...
        // outcome, turn
        turnExpireTime = Date.now() + BASE_TURN_EXPIRE_TIME + 2 * ALERT_TIME
        ...
        // outcome, tie, turn
        turnExpireTime = Date.now() + BASE_TURN_EXPIRE_TIME + 3 * ALERT_TIME
        serverEvent.content.pregameOutcome = outcomePregame
      } else {
    ...
    serverEvent.content.turnExpireTime = turnExpireTime
  ...
  socket.on("startGame", async ({ roomId }) => {
    ...
    try {
      ...
      const turnExpireTime = Date.now() + BASE_TURN_EXPIRE_TIME + 2 * ALERT_TIME
      startTimer(room, turnExpireTime);
      ...
      // // const turnExpireTime = Date.now() + 60000 // 1 minute
      // console.log('[startGame] turnExpireTime', turnExpireTime)
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            if (room.gamePhase === "pregame") {
              ...
              room.teams[user.team].moves[outcome]++; // need it to calculate if moves is empty

              // Add bonus throw on Yoot and Mo
              if (room.yootOutcome === 4 || room.yootOutcome === 5) {
                room.teams[user.team].throws++; // need it to calculate if team has no more throws
                room.gameLogs.push({
                  logType: 'throw',
                  content: {
                    playerName: user.name,
              ...
              (!room.rules.backdoLaunch && isBackdoMovesWithoutPieces(room.teams[user.team].moves.toObject(), room.teams[user.team].pieces))) ) {
                ...
                turnExpireTime = Date.now() + BASE_TURN_EXPIRE_TIME + 1 * ALERT_TIME
              }
            }
             ...
             * 
             * 
             * // post MVP: vary time by action that can be taken (move: longer, throw: shorter)
              // expire time depends on what will happen next
              base expire time + 5000 * numAlerts
              // in pregame
                // tie: alerts (outcome, tie, turn), throw
                // pass: alerts (outcome, turn), throw
                // win: alerts (outcome, winner, turn), throw
              // in game
  ...
  // handler
  // on action, set timer
  // if an action is made, clear it
  // else, trigger this event
  socket.on("timeExpired", async () => {
    console.log('[timeExpired]')
    // if during pregame, 
      // if the first team doesn't throw, pass the turn
      // if the second team doesn't throw, randomly decide the winner
    // if during game,
      // for now, pass turn
      // for later,
        // set 'automated': true
        // if team has throw, throw until it's 0
        // while !isMovesEmpty()
        // select a token
        // "move" or "score" with legalTiles
        // if you earn a throw
        // throw
        // repeat select and "move"/"score"
        // set 'automated': false
        // pass turn
  })

  // get room with findOne
  // access state and mutate it
  // call save on it
  socket.on("move", async ({ roomId, tile }) => {
    try {
      ...
      // on move - depends on the length of the jump animation
      // base expire time + alert_time * numAlerts + jump_time * spacesMoved
        // catch: jump, alerts (catch), throw
        // piggyback, no moves: jump, alerts (piggyback, turn)
        // piggyback, moves: jump, alerts (piggyback)
        // regular, no moves: jump, alerts (turn), throw
        // regular, moves: jump, move
      ...
      moves[moveUsed]--;
      turnExpireTime += (parseInt(Math.abs(moveUsed)) * JUMP_TIME)
      ...
      if (throws === 0 && isEmptyMoves(moves.toObject())) {
        ...
        room.teams[movingTeam].throws = throws
      }
      ...
      console.log('[move] turnExpireTime', turnExpireTime)
      startTimer(room, turnExpireTime)
      await room.save()
    } catch (err) {
